...
Модернизировать систему выравнивания текста и аудио, добавив:
- Асинхронную обработку задач через Celery + RabbitMQ
- Файловое хранилище MinIO с lifecycle management
- Многопользовательский режим с подписками
- API для воспроизведения аудио сегментов
- Централизованное логирование через OpenObserve

## Текущее состояние системы

### ✅ Что работает:
- FastAPI приложение с DDD архитектурой
- Домены: `alignment` и `models`
- Таблицы БД: `alignment_queue`, `languages`, `mfa_models`
- POST /alignment/ для создания задач (статус PENDING)
- CRUD операции для управления задачами

### ❌ Что не работает:
- Задачи не обрабатываются (остаются в PENDING)
- Нет системы пользователей
- Файлы хранятся локально без управления
- Нет API для воспроизведения аудио
- Не поднята инфраструктура в docker-compose
- Нет механизма очистки истекших файлов
...
### 🎯 Цель
Развернуть базовую инфраструктуру (RabbitMQ, MinIO, OpenObserve) и настроить простейшую Celery задачу.
...
- Celery worker успешно выполняет тестовую задачу `ping_task.delay()`
...
2. **Домен пользователей**
   - Создать `app/domains/users/` с моделями и сервисами
...
- GET /auth/me возвращает информацию о текущем пользователе
- GET /users/quota показывает лимиты и использование
...
1. **Docker Cron Scheduler**
   ...
   - Настроить volume для скриптов очистки
   - Создать cron schedule (ежедневно в 02:00)

2. **Скрипты очистки**
   - Создать `cleanup_expired_files.py` для удаления истекших файлов
   - Подключить к базе данных и MinIO из скрипта
...
- Квоты пользователей обновляются корректно
- Логи содержат информацию об очищенных файлах
